...
import type { LiquidityPoolCreateCallbacks } from '@/misc'
...
export function useNotifiedPoolCreationCallbacks(
    ...
    const onSend = React.useCallback<Required<LiquidityPoolCreateCallbacks>['onSend']>((message, params) => {
        onSendCallback?.(message, params)
        notify(
            <div className="notification-body">
                {intl.formatMessage({ id: 'LIQUIDITY_ADD_NOTIFICATION_PENDING_POOL_CREATING_NOTE' })}
            </div>,
            intl.formatMessage({ id: 'LIQUIDITY_ADD_NOTIFICATION_PENDING_POOL_CREATING_TITLE' }),
            {
                autoClose: false,
                closeOnClick: false,
                isLoading: true,
    ...
    const onTransactionSuccess = React.useCallback<Required<LiquidityPoolCreateCallbacks>['onTransactionSuccess']>(result => {
        onTransactionSuccessCallback?.(result)
        notify(
            <>
                <div className="notification-body">
                    {intl.formatMessage(
                        { id: 'LIQUIDITY_ADD_NOTIFICATION_SUCCESS_POOL_CREATING_NOTE' },
                    )}
                </div>
                <div
            ...
            intl.formatMessage({ id: 'LIQUIDITY_ADD_NOTIFICATION_SUCCESS_POOL_CREATING_TITLE' }),
            {
                closeOnClick: true,
                isLoading: false,
    ...
    const onTransactionFailure = React.useCallback<Required<LiquidityPoolCreateCallbacks>['onTransactionFailure']>(reason => {
        ...
        notify(
            ...
            intl.formatMessage({ id: 'LIQUIDITY_ADD_NOTIFICATION_FAILURE_POOL_CREATING_TITLE' }),
            {
                closeOnClick: true,
                isLoading: false,
