...
import { getFullContractState, TokenUtils, TokenWalletUtils } from '@/misc'
import type { Timeframe } from '@/modules/Charts/types'
import { OhlcvBarData, OhlcvKind } from '@/modules/Pools/types'
import type { OhlcvBar, PoolResponse } from '@/modules/Pools/types'
...
import { OhlcvData } from '@/modules/Charts/types'
...
export class PoolStore extends BaseStore<PoolStoreData, PoolStoreState> {
    ...
    protected async fetch(force?: boolean): Promise<void> {
        ...
        try {
            ...
            if (this.isNPool) {
                this.setState('graphType', PoolGraphType.TvlRatio)
            }
    ...
    public async fetchGraph(from?: number, to?: number): Promise<void> {
        ...
        try {
            ...
            if (this.graphType === PoolGraphType.Tvl || this.graphType === PoolGraphType.TvlRatio) {
                ohlcvKind = OhlcvKind.Tvl
            }
            else if (this.graphType === PoolGraphType.Volume) {
    ...
    public get customTokensDecimals(): PoolStoreData['customTokensDecimals'] {
        return this.data.customTokensDecimals
    }

    public get graph(): PoolStoreData['graph'] {
        return this.data.graph
    }

    public get lpToken(): PoolStoreData['lpToken'] {
    ...
    public get userLpBalance(): PoolStoreData['userLpBalance'] {
        return this.data.userLpBalance
    }

    public get graphType(): PoolStoreState['graphType'] {
        return this.state.graphType
    }

    public get isFetching(): PoolStoreState['isFetching'] {
        return this.state.isFetching
    }

    public get isFetchingGraph(): PoolStoreState['isFetchingGraph'] {
        return this.state.isFetchingGraph
    }

    public get isFetchingPrices(): PoolStoreState['isFetchingPrices'] {
    ...
    public get isSyncingLpToken(): PoolStoreState['isSyncingLpToken'] {
        return this.state.isSyncingLpToken
    }

    public get isSyncingUserData(): PoolStoreState['isSyncingUserData'] {
        return this.state.isSyncingUserData
    }

    public get notFound(): PoolStoreState['notFound'] {
        return this.state.notFound
    }

    public get timeframe(): PoolStoreState['timeframe'] {
    ...
    public get tokens(): PoolTokenData[] {
        return this.pool?.meta.currencyAddresses.map((address, idx) => {
            ...
            return {
                address,
                balance: new BigNumber(this.userLpBalance || 0)
                    .times(tvl || 0)
