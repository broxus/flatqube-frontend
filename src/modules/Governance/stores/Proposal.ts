...
import {
    ...
    ProposalState, ProposalType, TonAction, Vote,
...
export type TonActionData = {
    type: ProposalType.Default,
} | {
    type: ProposalType.NewCandidate | ProposalType.RemoveCandidate,
    gaugeData: GaugeItem,
}
...
export class ProposalStore {
    ...
    protected tonActionsTypeDisposer?: IReactionDisposer
    ...
    public init(proposalId: number): void {
        ...
        this.tonActionsTypeDisposer?.()
        this.tonActionsTypeDisposer = reaction(
            () => this.proposal,
            this.syncTonActionsData,
        )

        this.fetchDisposer?.()
        this.fetchDisposer = reaction(
            () => !this.wallet.isInitializing && !this.wallet.isConnecting,
            isReady => {
                if (isReady) {
                    this.userData.init()
                    this.fetch()
    ...
    protected async syncUserVote(): Promise<void> {
        try {
            if (this.proposalId && this.wallet.address) {
    ...
    static async getActionData(payload: string): Promise<TonActionData> {
        const staticRpc = useStaticRpc()

        try {
            const input = await staticRpc.rawApi.decodeInput({
                body: payload,
                abi: JSON.stringify(VoteEscrowAbi.Root),
                internal: true,
                method: 'approveGaugeDAO',
            })
            if (input?.method === 'approveGaugeDAO' && input.input.gauge) {
                return {
                    type: ProposalType.NewCandidate,
                    gaugeData: await gaugeHandler({}, {}, {
                        gaugeAddress: input.input.gauge.toString(),
                    })
                        .then(r => r.gauge),
                }
            }
        ...
        try {
            const input = await staticRpc.rawApi.decodeInput({
                ...
                abi: JSON.stringify(VoteEscrowAbi.Root),
                internal: true,
                method: 'removeGaugeDAO',
            })
            if (input?.method === 'removeGaugeDAO' && input.input.gauge) {
                return {
                    type: ProposalType.RemoveCandidate,
                    gaugeData: await gaugeHandler({}, {}, {
                        gaugeAddress: input.input.gauge.toString(),
                    })
                        .then(r => r.gauge),
    ...
    protected async syncTonActionsData(): Promise<void> {
        let tonActionsData: TonActionData[]

        if (this.proposal && this.proposal.actions?.tonActions) {
            tonActionsData = await Promise.all(
                this.proposal.actions.tonActions
                    .map(action => ProposalStore.getActionData(action.payload)),
            )
        }

        runInAction(() => {
            this._state.tonActionsData = tonActionsData
        })
    }
    ...
    public get tonActionsData(): TonActionData[] {
        return this._state.tonActionsData ?? []
    }
